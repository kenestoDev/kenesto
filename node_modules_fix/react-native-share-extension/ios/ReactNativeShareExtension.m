...
RCT_EXPORT_METHOD(openMainApp:(NSString *)appGroupId){
    [extensionContext completeRequestReturningItems:nil
                                  completionHandler:nil];
    [self openScheme:@"kenesto-main://"];
}

RCT_EXPORT_METHOD(deleteTempFiles:(NSString *)appGroupId) {
    [self cleanUpTempFiles:appGroupId];
}
...
typedef void (^ProviderCallback)(NSString *content, NSString *contentType, NSString* mimeType, NSNumber* fileSizeVal,  NSString* fileName, BOOL owner, NSError *err);
...
- (void)extractDataFromContext:(NSExtensionContext *)context withAppGroup:(NSString *) appGroupId andCallback:(void(^)(NSArray *items ,NSError *err))callback {
    @try {
        ...
        providerCb = ^ void (NSString *content, NSString *contentType, NSString* mimeType, NSNumber* fileSizeVal,  NSString* fileName, BOOL owner, NSError *err) {
            if (err) {
                callback(nil, err);
                return;
            }
...
- (void)extractDataFromProvider:(NSItemProvider *)provider withAppGroup:(NSString *) appGroupId andCallback:(void(^)(NSString* content, NSString* contentType, NSString* mimeType, NSNumber* fileSizeVal,  NSString* fileName, BOOL owner, NSError *err))callback {
    ...
    if([provider hasItemConformingToTypeIdentifier:@"public.image"]) {
        [provider loadItemForTypeIdentifier:@"public.image" options:nil completionHandler:^(id<NSSecureCoding, NSObject> item, NSError *error) {
            if (error) {
                callback(nil, nil, nil, nil, nil ,NO, error);
                return;
            }
                ...
                if ([item isKindOfClass: NSURL.class]) {
                    ...
                    [url getResourceValue:&fileSizeValue
                                   forKey:NSURLFileSizeKey
                                    error:&fileSizeError];
                    [url getResourceValue:&fileNameValue
                                   forKey:NSURLNameKey
                                    error:&fileSizeError];
                    ...
                    if (tempContainerURL == nil){
                        return callback(nil, nil, nil, nil, nil ,NO, nil);
                    }
                    
                    NSURL *tempFileURL = [tempContainerURL URLByAppendingPathComponent: fileName];
                    BOOL created = [fileData writeToFile:[tempFileURL path] atomically:YES];
                    if (created) {
                        return callback([tempFileURL absoluteString], @"public.image", mimeType, fileSizeValue, fileName, YES, nil);
                    } else {
                        return callback(nil, nil, nil, nil, nil, NO, nil);
                    }
                    ...
                    if (created) {
                        return callback([tempFileURL absoluteString], @"public.image",  nil, nil, nil ,YES, nil);
                    } else {
                        return callback(nil, nil,  nil, nil, nil ,NO, nil);
                    }
    ...
    if([provider hasItemConformingToTypeIdentifier:@"public.file-url"]) {
        [provider loadItemForTypeIdentifier:@"public.file-url" options:nil completionHandler:^(id<NSSecureCoding, NSObject> item, NSError *error) {
            ...
            {
                 url = [[NSURL alloc] initWithString:(NSString *)item];
            }
            NSNumber *fileSizeValue = nil;
            NSError *fileSizeError = nil;
            NSString * fileNameValue = @"";
            ...
            BOOL created = [fileData writeToFile:[tempFileURL path] atomically:YES];
            if (created) {
                return callback([tempFileURL absoluteString], @"public.file-url", mimeType, fileSizeValue, fileName , NO, nil);
            } else {
                return callback(nil, nil, nil, nil, nil, NO, nil);
            }
        }];
    ...
    if([provider hasItemConformingToTypeIdentifier:@"public.url"]) {
        [provider loadItemForTypeIdentifier:@"public.url" options:nil completionHandler:^(id<NSSecureCoding, NSObject> item, NSError *error) {
            ...
            BOOL created = [fileData writeToFile:[tempFileURL path] atomically:YES];
            if (created) {
                return callback([tempFileURL absoluteString], @"public.url", mimeType, fileSizeValue, fileName , NO, nil);
            } else {
                return callback(nil, nil, nil, nil, nil, NO, nil);
            }
        }];
    ...
    if([provider hasItemConformingToTypeIdentifier:@"public.plain-text"]) {
        [provider loadItemForTypeIdentifier:@"public.plain-text" options:nil completionHandler:^(id<NSSecureCoding, NSObject> item, NSError *error) {
            ...
            if ([item isKindOfClass:NSString.class]) {
                ...
                BOOL created = [fileData writeToFile:[tempFileURL path] atomically:YES];
                if (created) {
                    return callback([tempFileURL absoluteString], @"public.plain-text", mimeType, fileSizeValue, fileName , NO, nil);
                } else {
                    return callback(nil, nil, nil, nil, nil, NO, nil);
                }
                ...
                return callback([str string], @"public.plain-text",  nil, nil, nil ,NO, nil);
            } else if ([item isKindOfClass:NSData.class]) {
                NSString *str = [[NSString alloc] initWithData:(NSData *)item encoding:NSUTF8StringEncoding];
                if (str) {
                    return callback(str, @"public.plain-text", nil, nil, nil , NO, nil);
                } else {
                    return callback(nil, nil, nil, nil, nil , NO, nil);
                }
            } else {
    ...
    callback(nil, nil,  nil, nil, nil ,NO, nil);
}
