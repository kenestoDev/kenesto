...
//             };
             ...
             FPLogInfo(@"********************** filePath : %@", filePath);

             FPLogInfo(@"********************** filename : %@", filename);
            ...
            NSString *tempPath = [NSTemporaryDirectory() stringByAppendingPathComponent:[FPUtils genRandStringLength:20]];
             ...
             [imageData writeToURL:url
                       atomically:YES];
             
             mediaInfo.filename = filename;
             mediaInfo.mediaURL = url;
             mediaInfo.filesize = [NSNumber numberWithInt: [FPUtils fileSizeForLocalURL:url]];
             mediaInfo.key = filePath;
            
             successBlock(mediaInfo);
